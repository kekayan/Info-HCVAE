...
class PosteriorEncoder(nn.Module):
    ...
    def forward(self, c_ids, q_ids, a_ids):
        ...
        q_hs, q_state = self.encoder(q_embeddings, q_lengths.cpu())
        ...
        c_hs, c_state = self.encoder(c_embeddings, c_lengths.cpu())
        ...
        c_a_hs, c_a_state = self.encoder(c_a_embeddings, c_lengths.cpu())
...
class PriorEncoder(nn.Module):
    ...
    def forward(self, c_ids):
        ...
        c_embeddings = self.embedding(c_ids)
        c_hs, c_state = self.context_encoder(c_embeddings, c_lengths.cpu())
        c_h = c_state[0].view(self.nlayers, 2, -1, self.nhidden)[-1]
        c_h = c_h.transpose(0, 1).contiguous().view(-1, 2 * self.nhidden)
...
class AnswerDecoder(nn.Module):
    ...
    def forward(self, init_state, c_ids):
        ...
        M, _ = self.context_lstm(G, c_lengths.cpu())
...
class ContextEncoderforQG(nn.Module):
    ...
    def forward(self, c_ids, a_ids):
        ...
        c_outputs, _ = self.context_lstm(c_embeddings, c_lengths.cpu())
        # attention
...
class QuestionDecoder(nn.Module):
    ...
    def forward(self, init_state, c_ids, q_ids, a_ids):
        ...
        # question dec
        q_embeddings = self.embedding(q_ids)
        q_outputs, _ = self.question_lstm(q_embeddings, q_lengths.cpu(), init_state)
